...
@ApiTags("SalesmanagerMerchantContractStatus Query")
@UseGuards(SalesmanagerMerchantContractStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("salesmanagermerchantcontractstatuss/query")
export class SalesmanagerMerchantContractStatusQueryController {
  #logger = new Logger(SalesmanagerMerchantContractStatusQueryController.name);

  constructor(private readonly service: SalesmanagerMerchantContractStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all salesmanagermerchantcontractstatus with optional pagination" })
  @ApiResponse({ status: 200, type: SalesmanagerMerchantContractStatussResponse })
  @ApiQuery({ name: "options", required: false, type: SalesmanagerMerchantContractStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<SalesmanagerMerchantContractStatus>    
  ): Promise<SalesmanagerMerchantContractStatussResponse<SalesmanagerMerchantContractStatus>> {
    try {
     
      const salesmanagermerchantcontractstatuss = await this.service.findAll(options);
      logger.info("Retrieving all salesmanagermerchantcontractstatus");
  ...
  @ApiOperation({ summary: "Find salesmanagermerchantcontractstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter salesmanagermerchantcontractstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: SalesmanagerMerchantContractStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "SalesmanagerMerchantContractStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find salesmanagermerchantcontractstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<SalesmanagerMerchantContractStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all salesmanagermerchantcontractstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count salesmanagermerchantcontractstatuss with conditions" })
  @ApiResponse({ status: 200, type: SalesmanagerMerchantContractStatussResponse<SalesmanagerMerchantContractStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades SalesmanagerMerchantContractStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one salesmanagermerchantcontractstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one salesmanagermerchantcontractstatus or return error" })
  @ApiResponse({ status: 200, type: SalesmanagerMerchantContractStatusResponse<SalesmanagerMerchantContractStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad SalesmanagerMerchantContractStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get salesmanagermerchantcontractstatus by ID" })
  @ApiResponse({ status: 200, type: SalesmanagerMerchantContractStatusResponse<SalesmanagerMerchantContractStatus> })
  @ApiResponse({ status: 404, description: "SalesmanagerMerchantContractStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the salesmanagermerchantcontractstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<SalesmanagerMerchantContractStatusResponse<SalesmanagerMerchantContractStatus>> {
    try {
      const salesmanagermerchantcontractstatus = await this.service.findOne({ where: { id } });
      if (!salesmanagermerchantcontractstatus) {
        throw new NotFoundException(
          "SalesmanagerMerchantContractStatus no encontrado para el id solicitado"
        );
      }
      return salesmanagermerchantcontractstatus;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
