...
@ApiTags("CommissionMode Query")
@UseGuards(CommissionModeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("commissionmodes/query")
export class CommissionModeQueryController {
  #logger = new Logger(CommissionModeQueryController.name);

  constructor(private readonly service: CommissionModeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all commissionmode with optional pagination" })
  @ApiResponse({ status: 200, type: CommissionModesResponse })
  @ApiQuery({ name: "options", required: false, type: CommissionModeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<CommissionMode>    
  ): Promise<CommissionModesResponse<CommissionMode>> {
    try {
     
      const commissionmodes = await this.service.findAll(options);
      logger.info("Retrieving all commissionmode");
  ...
  @ApiOperation({ summary: "Find commissionmode by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter commissionmode', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: CommissionModesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "CommissionMode no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find commissionmodes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<CommissionMode>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all commissionmodes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count commissionmodes with conditions" })
  @ApiResponse({ status: 200, type: CommissionModesResponse<CommissionMode> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one commissionmode with conditions" })
  ...
  @ApiOperation({ summary: "Find one commissionmode or return error" })
  @ApiResponse({ status: 200, type: CommissionModeResponse<CommissionMode> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get commissionmode by ID" })
  @ApiResponse({ status: 200, type: CommissionModeResponse<CommissionMode> })
  @ApiResponse({ status: 404, description: "CommissionMode not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the commissionmode to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<CommissionModeResponse<CommissionMode>> {
    try {
      const commissionmode = await this.service.findOne({ where: { id } });
      if (!commissionmode) {
        throw new NotFoundException(
