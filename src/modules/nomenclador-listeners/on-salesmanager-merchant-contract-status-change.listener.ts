 ...
 * Licencia: MIT
 *
 * OnSalesmanagerMerchantContractStatusChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `salesmanager-merchant-contract-status` (owner: salesmanager). Suscribe a EventBus local y a Kafka.
 *
...
const NOM_KEY = 'salesmanager-merchant-contract-status';
const OWNER_BC = 'salesmanager';
...
const TOPICS: string[] = [
  'salesmanager-merchant-contract-status-created',
  'salesmanager-merchant-contract-status-updated',
  'salesmanager-merchant-contract-status-deleted',
];

@Injectable()
export class OnSalesmanagerMerchantContractStatusChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnSalesmanagerMerchantContractStatusChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnSalesmanagerMerchantContractStatusChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnSalesmanagerMerchantContractStatusChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onSalesmanagerMerchantContractStatusChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
