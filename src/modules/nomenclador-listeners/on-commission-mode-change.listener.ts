 ...
 * Licencia: MIT
 *
 * OnCommissionModeChangeListener — listener generado para reaccionar a cambios del
 * nomenclador `commission-mode` (owner: salesmanager). Suscribe a EventBus local y a Kafka.
 *
...
const TOPICS: string[] = [
  'commission-mode-created',
  'commission-mode-updated',
  'commission-mode-deleted',
];

@Injectable()
export class OnCommissionModeChangeListener
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(OnCommissionModeChangeListener.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    if (brokers.length === 0) {
      this.logger.debug(
        `KAFKA_BROKERS vacío — OnCommissionModeChangeListener solo escucha EventBus local.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `OnCommissionModeChangeListener suscrito a [${TOPICS.join(', ')}] (group ${groupId}, owner=${OWNER_BC}, cross=${CROSS_CONTEXT}).`,
      );
    } catch (err: any) {
      this.logger.warn(
  ...
  protected async onChange(eventName: string, payload: any): Promise<void> {
    this.logger.log(
      `[onCommissionModeChange] eventName=${eventName} payload=${JSON.stringify(payload).slice(0, 500)}`,
    );
  }
}
