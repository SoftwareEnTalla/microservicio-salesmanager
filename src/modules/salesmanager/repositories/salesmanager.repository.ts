...
import { logger } from '@core/logs/logger';
  ...
  export class SalesManagerRepository {
    ...
    async findAndCount(where?: Record<string, any>): Promise<[SalesManager[], number]> {
      logger.info('Ready to findByField SalesManager on repository:',where);
    ...
    client: LoggerClient.getInstance()
      ...
      .get(SalesManagerRepository.name),
        ...
        async findOne(options?: Record<string, any>): Promise<SalesManager | null> {
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) SalesManager:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<SalesManager> = options as FindOptionsWhere<SalesManager>;
    ...
    async update(
        ...
        logger.info('Updated instance of SalesManager with id:  was finded on repository:', instance);
